...
---
  一、上下文工程管理
    1.1 会话管理 — 三层递进压缩、自动触发、精确Token计数
    1.2 记忆管理 — 语义向量检索、相关性评分、淘汰策略
    1.3 知识管理 — 向量化存储、混合检索、知识版本化（最大差距）
    1.4 技能管理 — Prompt Tunables、效果评估、成本标签

  二、任务管理
    2.1 任务通信 — 异步通知、用量透传、子任务继续
    2.2 任务观测 — CostTracker、三层成本聚合、流式事件
    2.3 任务控制 — Token级预算、模型降级链、递减检测

  三、模型驱动管理
    3.1 模型选择 — Stage级路由（最大创新点，可降成本50-60%）
    3.2 成本追踪 — Prompt Caching（input降80-90%）、Cache命中率
    3.3 Prompt工程 — Tunables、版本管理、A/B测试
