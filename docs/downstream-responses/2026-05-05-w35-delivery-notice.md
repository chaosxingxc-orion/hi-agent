...
**Wave:** 35
**Status:** SHIP
**Manifest:** `2026-05-05-<manifest_id_pending>` (built at HEAD `8bce5bc`; manifest will be filled in by the release-captain run)
...
| Default-offline clean-env at HEAD | PASS | `docs/verification/<head>-default-offline-clean-env.json` (to be filled in by release-captain) — 9288 passed / 8 skipped / 0 failed (2:54 wall-clock) | measured | RIA standard |
...
| Real T3 (Volces) at HEAD | PENDING | `docs/delivery/<date>-<head>-t3-volces.json` — Volces re-run scheduled for closure-notice attachment; deferred to release-captain run because W35 lands hot-path code (run_manager.py, app.py, idempotency.py) per Rule 8 T3 invariance | pending | RIA standard |
| arch-7×24 fresh evidence | PENDING | `docs/verification/<head>-arch-7x24.json` 5/5 PASS at the W35 HEAD — to be regenerated by release-captain agent | pending | RIA standard |
...
- Manifest (pending): `docs/releases/platform-release-manifest-2026-05-05-<head>.json`
...
**T3 evidence:** PENDING — Volces real-LLM T3 re-run scheduled for the release-captain agent. W35 lands hot-path code in `hi_agent/server/run_manager.py`, `hi_agent/server/app.py`, `hi_agent/server/idempotency.py`, and `agent_server/runtime/lifespan.py`, so T3 invariance demands a fresh gate run at the W35 HEAD per CLAUDE.md Rule 8. Evidence file path will be `docs/delivery/<date>-<head>-t3-volces.json`.
...
**Audit head:** docs match hi-agent main at 2026-05-05, manifest release_head `<pending release-captain run>`
