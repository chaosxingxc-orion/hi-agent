...
| S2 | No contract between the four execution paths (`execute` / `execute_graph` / `execute_async` / `_execute_remaining`). Each hand-rolls try/except, finalization, session state, hook wrapping. Every feature drifts in 3 of 4. | P-2, P-3 |
| S3 | Store factories proliferate without a registry. Each new subsystem builds its own `LongTermMemoryGraph` / `ShortTermMemoryStore` instance; there is no per-profile cache. New consumers reliably produce new duplication bugs. | P-4 |
