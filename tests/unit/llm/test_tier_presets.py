"""Unit tests for apply_research_defaults in tier_presets.

Verifies that research pipeline purpose → tier mappings are correctly
...
def test_pi_agent_is_strong() -> None:
    """pi_agent purpose should map to strong tier after applying research defaults."""
    router = _make_router()
...
def test_survey_fetch_is_light() -> None:
    """survey_fetch purpose should map to light tier after applying research defaults."""
    router = _make_router()
    apply_research_defaults(router)
...
def test_existing_perception_not_clobbered() -> None:
    """Applying research defaults must not change the built-in perception mapping."""
    router = _make_router()
...
def test_existing_control_not_clobbered() -> None:
    """Applying research defaults must not change the built-in control mapping."""
    router = _make_router()
