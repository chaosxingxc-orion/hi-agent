...
class TestInMemoryPersistence:
    ...
    def test_wal_mode(self) -> None:
        cb = CircuitBreaker(db_path=":memory:")
        row = cb._db.execute("PRAGMA journal_mode").fetchone()  # type: ignore[union-attr]  expiry_wave: Wave 29
    ...
    def test_failure_persisted(self) -> None:
        ...
        cb.mark_failure("cap")
        row = cb._db.execute(  # type: ignore[union-attr]  expiry_wave: Wave 29
...
class TestFilePersistence:
    ...
    def test_open_state_survives_restart(self, tmp_path: pytest.TempPathFactory) -> None:
        db_file = str(tmp_path / "cb.db")  # type: ignore[operator]  expiry_wave: Wave 29
    ...
    def test_closed_state_not_reloaded(self, tmp_path: pytest.TempPathFactory) -> None:
        db_file = str(tmp_path / "cb2.db")  # type: ignore[operator]  expiry_wave: Wave 29
    ...
    def test_only_open_loaded_on_init(self, tmp_path: pytest.TempPathFactory) -> None:
        ...
        db_file = str(tmp_path / "cb3.db")  # type: ignore[operator]  expiry_wave: Wave 29
    ...
    def test_multiple_named_circuits_survive_restart(
        ...
        db_file = str(tmp_path / "cb4.db")  # type: ignore[operator]  expiry_wave: Wave 29
