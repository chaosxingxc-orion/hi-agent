...
def test_append_rejects_non_mapping(store: SqliteDecisionAuditStore) -> None:
    with pytest.raises(TypeError):
        store.append(["run_id", "stage_id"])  # type: ignore[arg-type]  expiry_wave: permanent
...
def test_latest_by_stage_scoped_to_run(store: SqliteDecisionAuditStore) -> None:
    ...
    assert store.latest_by_stage("r1", "s1")["seq"] == 10  # type: ignore[index]  expiry_wave: permanent
