...
def test_workspace_skill_overrides_global(tmp_path):
    ...
    lit = next(
        (
            s
            for s in skills
            if "lit-review" in (s.skill_id or "").lower() or "lit-review" in (s.name or "").lower()
        ),
        None,
    )
    ...
    content_str = (
        lit.raw_content if hasattr(lit, "raw_content") else getattr(lit, "content", str(lit))
    )
    if content_str is None:
        content_str = str(lit)
    assert (
        "version: 2" in content_str
        or getattr(lit, "version", None) == "2"
        or getattr(lit, "source", None) == "workspace"
    ), (
        "Expected workspace version to win, "
        f"got content={content_str!r}, source={getattr(lit, 'source', None)!r}"
    )
...
def test_global_skill_used_when_no_workspace_override(tmp_path):
    ...
    (global_dir / "analysis" / "SKILL.md").write_text("# analysis\nversion: 1\n\nAnalysis skill\n")
    ...
    analysis = next(
        (
            s
            for s in skills
            if "analysis" in (s.skill_id or "").lower() or "analysis" in (s.name or "").lower()
        ),
        None,
    )
...
def test_workspace_only_skill_included(tmp_path):
    ...
    ws_skill = next(
        (
            s
            for s in skills
            if "ws-only-skill" in (s.skill_id or "").lower()
            or "ws-only-skill" in (s.name or "").lower()
        ),
        None,
    )
