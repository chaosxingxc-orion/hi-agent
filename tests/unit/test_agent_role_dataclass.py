...
def test_team_run_immutable():
    ...
    team = TeamRun(team_id="t1", pi_run_id="run-pi", project_id="proj-1")
    with pytest.raises((AttributeError, TypeError)):
...
def test_team_shared_context_defaults():
    ...
    ctx = TeamSharedContext(team_id="t1", project_id="proj-1")
...
def test_team_run_default_empty_members():
    ...
    team = TeamRun(team_id="t2", pi_run_id="run-pi-2", project_id="proj-2")
