...
def test_agent_role_immutable():
    ...
    with pytest.raises((AttributeError, TypeError)):
        role.role_id = "other"  # type: ignore[misc]  expiry_wave: Wave 30
...
def test_team_run_immutable():
    ...
    with pytest.raises((AttributeError, TypeError)):
        team.team_id = "other"  # type: ignore[misc]  expiry_wave: Wave 30
...
def test_team_shared_context_immutable():
    ...
    ctx = TeamSharedContext(team_id="t1", project_id="proj-1", tenant_id="test-tenant")
    with pytest.raises((AttributeError, TypeError)):
        ctx.project_id = "other"  # type: ignore[misc]  expiry_wave: Wave 30
