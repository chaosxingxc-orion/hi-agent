...
def test_run_store_workspace_filter_excludes_other_user(run_store):
    record = RunRecord(
        run_id="r2", tenant_id="t1", user_id="u1", session_id="s1",
        task_contract_json="{}", status="queued", priority=5,
        attempt_count=0, cancellation_flag=False,
...
def test_event_store_has_workspace_columns(event_store):
    evt = StoredEvent(
        event_id="e1", run_id="r1", sequence=1,
        event_type="test", payload_json="{}",
        tenant_id="t1", user_id="u1", session_id="s1",
    )
    event_store.append(evt)
    events = event_store.list_since(run_id="r1", last_id=0, tenant_id="t1", user_id="u1", session_id="s1")
    assert len(events) == 1
