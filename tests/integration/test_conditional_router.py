...
class TestErrorResilience:
    ...
    def test_failing_condition_skipped_in_evaluate(self) -> None:
        router = ConditionalRouter()
        router.add_route(
            condition=lambda ctx: 1 / 0 > 0,  # type: ignore[operator]  expiry_wave: Wave 26
    ...
    def test_all_conditions_fail_uses_default(self) -> None:
        router = ConditionalRouter()
        router.add_route(
            condition=lambda ctx: (_ for _ in ()).throw(ValueError("boom")),  # type: ignore[func-returns-value]  expiry_wave: Wave 26
            target_stage="S_crash",
        )
