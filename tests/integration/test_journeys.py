...
def test_journey_execute_gate_approve() -> None:
    ...
    executor._execute_stage = patched_execute_stage  # type: ignore[method-assign]  expiry_wave: Wave 27
...
def test_journey_combined_pi_c_pi_d(tmp_path: Path) -> None:
    ...
    executor._execute_stage = gated_execute_stage  # type: ignore[method-assign]  expiry_wave: Wave 27
...
@pytest.mark.skipif(
    ...
    reason="K-13: PI-C+PI-D combination requires live kernel + LLM (VOLCES_API_KEY) — add to E2E suite when available",  # noqa: E501  # expiry_wave: Wave 27
)
