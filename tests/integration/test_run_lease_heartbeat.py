...
def test_heartbeat_keeps_lease_alive_during_long_run() -> None:
    ...
    rq.heartbeat = _spy_heartbeat  # type: ignore[method-assign]  expiry_wave: Wave 17
...
def test_heartbeat_failure_transitions_run_to_failed() -> None:
    ...
    rq.heartbeat = _deny_heartbeat  # type: ignore[method-assign]  expiry_wave: Wave 17

    # Use a very short heartbeat interval so the test completes quickly.
    # We monkey-patch the property value directly.
    rq.__dict__["lease_heartbeat_interval_seconds"] = 0.1  # type: ignore[attr-defined]  expiry_wave: Wave 17
