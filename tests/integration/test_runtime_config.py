...
def test_runtime_config_patch_updates_version_and_snapshot() -> None:
    ...
    manager = RuntimeConfigManager(initial={"a": 1}, history=ConfigHistory(), now_fn=lambda: 50.0)
...
def test_runtime_config_history_records_multiple_changes() -> None:
    ...
    manager = RuntimeConfigManager(history=ConfigHistory(), now_fn=timestamps)
...
def test_runtime_config_rejects_empty_actor_or_patch() -> None:
    ...
    manager = RuntimeConfigManager(history=ConfigHistory())
