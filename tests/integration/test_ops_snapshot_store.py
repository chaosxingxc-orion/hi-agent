...
def test_append_and_latest_and_list_run_are_deterministic() -> None:
    ...
    store.append({"run_id": "run-1", "tenant_id": "t-a", "timestamp": 10.0, "status": "ok"})
    store.append({"run_id": "run-2", "tenant_id": "t-a", "timestamp": 20.0, "status": "warn"})
    store.append({"run_id": "run-1", "tenant_id": "t-a", "timestamp": 30.0, "status": "error"})

    latest = store.latest("run-1", "t-a")
    assert latest is not None
    assert latest["timestamp"] == 30.0
    assert latest["status"] == "error"

    run_entries = store.list_run("run-1", "t-a")
...
def test_list_all_supports_limit_and_descending_order() -> None:
    ...
    store = OpsSnapshotStore()
    store.append({"run_id": "run-1", "tenant_id": "t-a", "timestamp": 10.0})
    store.append({"run_id": "run-2", "tenant_id": "t-a", "timestamp": 30.0})
    store.append({"run_id": "run-3", "tenant_id": "t-a", "timestamp": 20.0})
...
def test_store_is_copy_safe_for_append_and_reads() -> None:
    ...
    store = OpsSnapshotStore()
    payload = {"run_id": "run-copy", "tenant_id": "t-a", "timestamp": 1.0, "meta": {"a": 1}}
    stored = store.append(payload)
    payload["meta"]["a"] = 999
    stored["meta"]["a"] = 888

    latest = store.latest("run-copy", "t-a")
    assert latest is not None
...
def test_append_validation_errors(run_id: str, timestamp: object) -> None:
    ...
    with pytest.raises(ValueError):
        store.append({"run_id": run_id, "tenant_id": "t-a", "timestamp": timestamp})
