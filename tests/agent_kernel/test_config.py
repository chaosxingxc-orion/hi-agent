...
class TestFrozen:
    ...
    def test_assignment_raises(self) -> None:
        ...
        with pytest.raises(FrozenInstanceError):
            cfg.max_tracked_runs = 999  # type: ignore[misc]  expiry_wave: Wave 17

    def test_assignment_on_str_field_raises(self) -> None:
        """Verifies assignment on str field raises."""
        cfg = KernelConfig()
        with pytest.raises(FrozenInstanceError):
            cfg.default_model_ref = "other"  # type: ignore[misc]  expiry_wave: Wave 17

    def test_assignment_on_optional_field_raises(self) -> None:
        """Verifies assignment on optional field raises."""
        cfg = KernelConfig()
        with pytest.raises(FrozenInstanceError):
            cfg.phase_timeout_s = 10.0  # type: ignore[misc]  expiry_wave: Wave 17
