...
def _make_gateway(lifecycle_state: str = "running", waiting_external: bool = False) -> MagicMock:
    ...
    gw.query_projection = AsyncMock(
        return_value=RunProjection(
            run_id="run-1",
            lifecycle_state=lifecycle_state,  # type: ignore[arg-type]  expiry_wave: Wave 26
            projected_offset=0,
        ...
        asyncio.run(
            facade.mark_branch_state(
                BranchStateUpdateRequest(
                    run_id="run-1",
                    branch_id="b1",
                    new_state="pruned",  # type: ignore[arg-type]  expiry_wave: Wave 26
                )
            )
        )
        with facade._branch_lock:
        ...
        asyncio.run(
            facade.mark_branch_state(
                BranchStateUpdateRequest(
                    run_id="run-1",
                    branch_id="b1",
                    new_state="failed",  # type: ignore[arg-type]  expiry_wave: Wave 26
    ...
    def test_all_gate_types_accepted(self):
        ...
        for gate_type in (
            ...
            req = HumanGateRequest(
                ...
                gate_type=gate_type,  # type: ignore[arg-type]  expiry_wave: Wave 26
                run_id="run-1",
                trigger_reason="test",
                trigger_source="system",
            )
