...
class TestReconciliationAction:
    ...
    def test_is_frozen_dataclass(self) -> None:
        ...
        with pytest.raises((FrozenInstanceError, AttributeError)):
            action.action_taken = "skipped"  # type: ignore[misc]  expiry_wave: permanent
...
class TestDispatchOutboxReconcilerSync:
    ...
    def test_orphaned_unknown_effect_key_skipped(self) -> None:
        ...
        assert store.get(env.dispatch_idempotency_key).state == "unknown_effect"  # type: ignore[union-attr]  expiry_wave: permanent
    ...
    def test_orphaned_acknowledged_key_skipped(self) -> None:
        ...
        action = result.actions[0]
        assert action.action_taken == "skipped"
        assert store.get(env.dispatch_idempotency_key).state == "acknowledged"  # type: ignore[union-attr]  expiry_wave: permanent
    ...
    def test_multiple_violations_all_repaired(self) -> None:
        ...
        assert store.get(env_a.dispatch_idempotency_key).state == "unknown_effect"  # type: ignore[union-attr]  expiry_wave: permanent
    ...
    def test_dedupe_state_error_during_repair_skipped(self) -> None:
        ...
        store.mark_dispatched = _failing_mark_dispatched  # type: ignore[method-assign]  expiry_wave: permanent
        ...
        store.mark_dispatched = original_mark_dispatched  # type: ignore[method-assign]  expiry_wave: permanent
...
class TestDispatchOutboxReconcilerAsync:
    ...
    async def test_orphaned_reserved_key_async(self) -> None:
        ...
        action = result.actions[0]
        assert action.action_taken == "marked_unknown_effect"
        assert store.get(key).state == "unknown_effect"  # type: ignore[union-attr]  expiry_wave: permanent
    ...
    async def test_dedupe_state_error_skipped_async(self) -> None:
        ...
        store.mark_dispatched = _fail  # type: ignore[method-assign]  expiry_wave: permanent
