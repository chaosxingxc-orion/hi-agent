...
async def test_structured_compressor_calls_llm():
    ...
    _new_messages, summary = await compressor.compress(messages, existing_summary=None)
...
async def test_structured_compressor_returns_compressed_messages():
    ...
    messages = _make_mixed_messages(10)
    new_messages, _summary = await compressor.compress(messages)
...
async def test_structured_compressor_incremental_update():
    ...
    messages = _make_mixed_messages(10)
    _new_messages, merged_summary = await compressor.compress(messages, existing_summary=existing)
