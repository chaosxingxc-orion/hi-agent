# TRACE Enterprise Agent Architecture Review V1.2.1

> Status: Architecture review draft V1.2.1
> Scope: Enterprise agent architecture, validated first in scientific-research tasks
> Repository: `D:\chao_workspace\hi-agent`
> Revision note: V1.2.1 keeps the V1.2 architecture stable and adds three review-closing refinements: `ActionState.succeeded`, model-layer failure taxonomy, and side-effect class boundary principle.
...
`CTS` is not an unbounded search space. In V1.2, it is explicitly governed by an exploration budget.
...
In V1.2, `Evolve` is no longer treated as a generic aspiration. It is a bounded post-execution subsystem.
...
Each evolve execution must declare an explicit `change_scope`.
...
One evolve execution must not update evaluation baselines and the evaluated targets in the same change set.
...
`Evolve` itself must also be budgeted.

Recommended controls:

- `max_runs_analyzed_per_batch_evolution`
- `max_llm_calls_per_evolve_execution`
- `max_summary_tokens_per_evolve_execution`
- `min_regression_trigger_cooldown`

#### Skill crystallization path

V1.2 adopts a five-stage lifecycle:
...
- `Deprecated Skill`: no longer recommended for new runs
- `Retired Skill`: removed from active routing and preserved only for audit or replay compatibility

This prevents one-off success from being treated as a stable skill.

Each skill should eventually carry:
...
#### Completeness check and degradation

Before a model call, Task View assembly must perform a completeness check.

If required information cannot fit within the current budget, the system must not silently continue after truncation.

Allowed degradation actions:

- trigger a human gate
...
V1.2 defines four explicit human gates:
...
Human gates are not only user-invoked. The system may actively trigger them.

- `Human Gate A`: trigger when the task contract conflicts with newly captured evidence
- `Human Gate B`: trigger when exploration budget is heavily consumed and no viable branch is available
- `Human Gate C`: trigger when intermediate artifact quality is below threshold or contradictory evidence is detected
- `Human Gate D`: trigger for final high-risk package approval or irreversible external submission preparation
...
3. agent-kernel
   - persists run state
   - packages the selected Task View into stable replayable model invocation records through LLM Gateway contract
...
To avoid repository boundary drift:
...
- `agent-core` only supplies capabilities and resources, and does not perform task-level cognitive assembly

### 6.7 Runtime Truth Model

TRACE requires an explicit kernel-facing runtime truth model.
...
```text
RunState:
  created -> active -> waiting -> recovering -> completed
                            |-> failed
                            |-> aborted
...
### 6.8 Idempotency and Side-Effect Contract

Because TRACE performs real environment operations, idempotency is an architecture concern, not an implementation afterthought.

At minimum, the following identifiers should exist:
...
Side-effect class should be determined by the blast radius and operational impact of the action, not by the network location of the target.

#### Contract rules

- every external action must carry an idempotency key
- callback handling must be idempotent
- retry policy must respect side-effect class
- irreversible actions must be reviewable and auditable before execution
- compensatable and non-compensatable actions must be distinguishable in runtime records

### 6.9 Failure Taxonomy

To support postmortem analysis, route pruning, and evolution triggers, V1.2 defines an initial failure taxonomy:
...
#### High-risk side-effect governance

Harness-governed actions should distinguish at least four operational classes:
...
- approval-required marking for high-risk actions
- compensatable vs non-compensatable action flags
...
V1.2 adds one more requirement:

- old runs must remain replayable against the versions of route policy, skill policy, and evaluation policy that were active when the decision was made
...
### 11.5 Idempotency and Recovery Requirements

- the kernel must support idempotent callback consumption
- the kernel must record action identity and attempt identity separately
- the kernel must support side-effect-aware retry policies
- the kernel must distinguish compensatable and irreversible actions
- the kernel should preserve enough data to support recovery arbitration when callbacks, timeouts, and retries race

### 11.6 Harness Requirements
...
### 11.8 Versioning and Isolation Requirements

- the kernel should support version pinning for decisions made by older runs
- the kernel should expose enough metadata for change-scope isolation during evolve rollouts
- the kernel should avoid forcing online runs to adopt newly evolved policies mid-decision without explicit opt-in
...
In parallel, one shadow validation scenario should be prepared outside the research domain to reduce domain lock-in risk.

Suggested shadow scenario:
