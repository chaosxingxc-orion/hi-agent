...
try:
    ...
    _otel_trace = None  # type: ignore[assignment]  # expiry_wave: Wave 30
    _NonRecordingSpan = None  # type: ignore[assignment,misc]  # expiry_wave: Wave 30
    _PROPAGATOR = None  # type: ignore[assignment]  # expiry_wave: Wave 30
...
def _extract_otel_context(trace_context: str | None) -> Any | None:
    ...
    except Exception:  # rule7-exempt: expiry_wave="Wave 30" pylint: disable=broad-exception-caught
        return None
...
try:
    from opentelemetry import (
        metrics as _otel_metrics,  # type: ignore[attr-defined]  # expiry_wave: Wave 30
    )

    _OTEL_METRICS_AVAILABLE = True
except ImportError:  # pragma: no cover
    _OTEL_METRICS_AVAILABLE = False
    _otel_metrics = None  # type: ignore[assignment]  # expiry_wave: Wave 30
