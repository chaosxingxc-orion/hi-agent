...
class SQLiteConnectionPool:
    ...
    def release_read(self, conn: sqlite3.Connection) -> None:
        ...
        if self._closed:
            with contextlib.suppress(Exception):  # rule7-exempt: connection close on pool teardown; best-effort
                conn.close()
            return
    ...
    def close_all(self) -> None:
        ...
        with contextlib.suppress(Exception):  # rule7-exempt: SQLite connection close on shutdown; best-effort teardown
        ...
        while True:
            try:
                ...
                break
            with contextlib.suppress(Exception):  # rule7-exempt: SQLite connection close on pool drain; best-effort
                conn.close()
