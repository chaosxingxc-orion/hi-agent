...
class SQLiteConnectionPool:
    ...
    def release_read(self, conn: sqlite3.Connection) -> None:
        ...
        if self._closed:
            with contextlib.suppress(Exception):  # rule7-exempt: connection close on pool teardown; best-effort  # noqa: E501  # expiry_wave: permanent  # added: W25 baseline sweep
                conn.close()
            return
    ...
    def close_all(self) -> None:
        ...
        with self._read_lock:
            if self._closed:
                return
            self._closed = True
        with contextlib.suppress(Exception):  # rule7-exempt: SQLite WAL checkpoint on close; best-effort teardown  # noqa: E501  # expiry_wave: permanent  # added: W25 baseline sweep
            self._write_conn.execute("PRAGMA wal_checkpoint(TRUNCATE)")
        with contextlib.suppress(Exception):  # rule7-exempt: SQLite connection close on shutdown; best-effort teardown  # noqa: E501  # expiry_wave: permanent  # added: W25 baseline sweep
        ...
        while True:
            try:
                ...
                break
            with contextlib.suppress(Exception):  # rule7-exempt: SQLite connection close on pool drain; best-effort  # noqa: E501  # expiry_wave: permanent  # added: W25 baseline sweep
                conn.close()
