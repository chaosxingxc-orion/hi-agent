    ...
    def write(self, record: TaskViewRecord) -> None:
        ...
        with self._lock:
            try:
                ...
                with contextlib.suppress(Exception):  # rule7-exempt: SQLite ROLLBACK on error path; must not mask original exception  # noqa: E501  # expiry_wave: Wave 27  # added: W25 baseline sweep
                    self._conn.rollback()
                raise
