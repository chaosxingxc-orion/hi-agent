    ...
    def mark_branch_state(
        ...
        self._call(
            ...
            BranchStateUpdateRequest(
                run_id=normalized_run,
                branch_id=normalized_branch,
                new_state=normalized_state,  # type: ignore[arg-type]  expiry_wave: Wave 26
    ...
    def open_human_gate(self, request: HumanGateRequest) -> None:
        ...
        self._call(
            ...
            KernelHumanGateRequest(
                ...
                gate_type=self._non_empty(request.gate_type, "gate_type"),  # type: ignore[arg-type]  expiry_wave: Wave 26
                run_id=self._non_empty(request.run_id, "run_id"),
                trigger_reason=str(
                    context.get("trigger_reason") or context.get("reason") or "human_gate_requested"
                ),
                trigger_source=str(context.get("trigger_source") or "system"),  # type: ignore[arg-type]  expiry_wave: Wave 26
