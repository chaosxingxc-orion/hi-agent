    ...
    async def wrap_llm_call(
        ...
        try:
            if inspect.iscoroutinefunction(call_fn):
                ...
                response = call_fn(ctx.request)  # type: ignore[assignment]  expiry_wave: Wave 27
        except Exception as exc:
            _hooks_errors_total.inc()
            error = exc
        finally:
        ...
        llm_result = LLMCallResult(
            context=ctx,
            response=response,  # type: ignore[arg-type]  expiry_wave: Wave 27
            duration_ms=duration_ms,
        )
    ...
    async def wrap_tool_call(
        ...
        try:
            if inspect.iscoroutinefunction(call_fn):
                ...
                result_str = call_fn(ctx)  # type: ignore[assignment]  expiry_wave: Wave 27
        except Exception as exc:
            _hooks_errors_total.inc()
            error = exc
        finally:
