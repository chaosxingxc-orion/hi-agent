...
from agent_kernel.kernel.task_manager.contracts import (  # noqa: F401  expiry_wave: permanent
    ExhaustedPolicy,
    TaskAttempt,
    TaskRestartPolicy,
)
...
__all__ = [
    ...
    "RestartPolicyEngine",
    "TaskAttempt",
    "TaskAttemptRecord",  # noqa: F822 — provided via module __getattr__  expiry_wave: permanent
    "TaskRestartPolicy",
]
...
class RestartPolicyEngine:
    ...
    def _decide(
        ...
        if retryability == "non_retryable":
            action: RestartAction = policy.on_exhausted  # type: ignore[assignment]  expiry_wave: permanent
            return RestartDecision(
                task_id=task_id,
                action=action,
                next_attempt_seq=None,
        ...
        return RestartDecision(
            task_id=task_id,
            action=on_exhausted,  # type: ignore[arg-type]  expiry_wave: permanent
            next_attempt_seq=None,
            reason=(
