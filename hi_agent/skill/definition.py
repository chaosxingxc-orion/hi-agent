...
def _parse_yaml_value(raw: str) -> str | int | float | bool | list[str]:
    ...
    try:
        if "." in val:
            return float(val)
        return int(val)
    except ValueError:  # rule7-exempt: expiry_wave="permanent" replacement_test: wave22-tests
        pass
...
class SkillDefinition:
    ...
    def from_markdown(cls, content: str, source_path: str = "") -> SkillDefinition:
        ...
        def _float(key: str, default: float) -> float:
            ...
            try:
                return float(v)  # type: ignore[arg-type]  expiry_wave: permanent
        ...
        def _int(key: str, default: int) -> int:
            v = meta.get(key, default)
            try:
                return int(v)  # type: ignore[arg-type]  expiry_wave: permanent
            except (ValueError, TypeError):
                return default
