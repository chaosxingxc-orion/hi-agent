"""Per-run retrospective analysis for the Evolve subsystem."""
...
class PostmortemAnalyzer:
    ...
    def __init__(self, llm_gateway: LLMGateway | None = None) -> None:
        """Initialize the retrospective analyzer.
    ...
    def analyze(self, postmortem: RunRetrospective) -> EvolveResult:
        ...
        Args:
            postmortem: Structured retrospective data for the run.
        ...
        if self._llm is not None:
            try:
                ...
                record_fallback(
                    "heuristic",
                    reason="retrospective_llm_analyze_failed",
    ...
    def _llm_analyze(self, postmortem: RunRetrospective) -> tuple[list[EvolveChange], int]:
        """Use LLM to extract deeper improvement signals from the retrospective.
        ...
        prompt = _build_retrospective_prompt(postmortem)
        request = LLMRequest(
        ...
        response = self._llm.complete(request)  # type: ignore[union-attr]  expiry_wave: permanent
        changes = _parse_llm_changes(response.content, postmortem.run_id)
        return changes, 1

    def _rule_based_analysis(self, postmortem: RunRetrospective) -> list[EvolveChange]:
        """Apply rule-based heuristics to extract improvement signals.

        Args:
            postmortem: Structured retrospective data.
    ...
    def _detect_failure_patterns(self, postmortem: RunRetrospective) -> list[EvolveChange]:
    ...
    def _assess_branch_efficiency(self, postmortem: RunRetrospective) -> list[EvolveChange]:
...
def _build_retrospective_prompt(postmortem: RunRetrospective) -> str:
    """Build a structured prompt for LLM retrospective analysis."""
    lines = [
        "You are an AI agent improvement advisor. Analyze this run retrospective and "
...
def _parse_llm_changes(content: str, run_id: str) -> list[EvolveChange]:
    ...
    try:
        ...
        _logger.warning("retrospective._parse_llm_changes: invalid JSON from LLM: %s", exc)
        try:
            from hi_agent.observability.fallback import record_fallback
            ...
            record_fallback(
                ...
                extra={
                    "site": "retrospective._parse_llm_changes",
                    "error_type": type(exc).__name__,
                    "error": str(exc)[:200],
