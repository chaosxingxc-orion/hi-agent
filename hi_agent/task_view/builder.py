    ...
    *,
    ...
    if stage_summaries is not None:
        return _legacy_build(
            run_index=run_index,  # type: ignore[arg-type]  expiry_wave: Wave 26
            stage_summaries=stage_summaries,
            episodes=episodes or [],  # type: ignore[arg-type]  expiry_wave: Wave 26
    ...
    if episodes and remaining > 0:
        ...
        raw = format_episodes(episodes, layer_max)  # type: ignore[arg-type]  expiry_wave: Wave 26
        if raw:
            content = enforce_layer_budget(raw, layer_max)
            tokens = count_tokens(content)
            sections.append(
