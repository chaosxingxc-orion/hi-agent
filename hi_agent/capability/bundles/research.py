...
def _allow_heuristic() -> bool:
    ...
    return env != "prod" or os.environ.get("HI_AGENT_ALLOW_HEURISTIC_FALLBACK", "").lower() in ("1", "true", "yes")
...
class ResearchBundle(CapabilityBundle):
    ...
    def __init__(self, llm_gateway: "LLMGateway | None" = None) -> None:
        """Initialize with optional LLM gateway.

        Args:
            llm_gateway: LLM gateway to use for real calls. When None, all
                capabilities use heuristic fallback (non-prod mode only).
        """
        ...
        specs = [
            CapabilitySpec(
                ...
                handler=_llm_or_heuristic(
                    self._gateway,
                    "web_search",
                    "You are a web search engine. Given a query, return relevant results as JSON: "
            ...
            CapabilitySpec(
                ...
                handler=_llm_or_heuristic(
                    self._gateway,
                    "web_extract",
                    "You are a web content extractor. Extract the main text content from the given URL or HTML. "
                    'Return JSON: {"title": "...", "content": "...", "url": "...", "word_count": 0}',
            ...
            CapabilitySpec(
                ...
                handler=_llm_or_heuristic(
                    self._gateway,
                    "paper_parse",
                    "You are an academic paper parser. Extract structured metadata from an academic paper. "
                    'Return JSON: {"title": "...", "authors": [], "abstract": "...", "keywords": [], '
                    '"year": null, "venue": "...", "doi": null}',
            ...
            CapabilitySpec(
                ...
                handler=_llm_or_heuristic(
                    self._gateway,
                    "citation_capture",
                    "You are a citation extractor. Extract and normalize all citations from the given text. "
                    'Return JSON: {"citations": [{"text": "...", "normalized": "...", "doi": null}]}',
            ...
            CapabilitySpec(
                ...
                handler=_llm_or_heuristic(
                    self._gateway,
                    "summarize_sources",
                    "You are a research summarizer. Synthesize a concise summary from multiple sources. "
                    'Return JSON: {"summary": "...", "key_points": [], "source_count": 0, "confidence": 0.0}',
            ...
            CapabilitySpec(
                ...
                handler=_llm_or_heuristic(
                    self._gateway,
                    "literature_review",
                    "You are a literature review specialist. Produce a structured literature review for the given topic. "
                    'Return JSON: {"topic": "...", "sections": [{"heading": "...", "content": "..."}], '
                    '"gaps": [], "total_sources": 0}',
