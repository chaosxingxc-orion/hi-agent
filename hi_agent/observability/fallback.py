...
def _increment_no_run_scope_counter() -> None:
    ...
    except Exception:  # rule7-exempt: expiry_wave="permanent" metrics must never crash callers
        pass
...
def record_fallback(
    ...
    *,
    ...
    with contextlib.suppress(Exception):  # pragma: no cover; rule7-exempt: logging must not crash callers  # noqa: E501  expiry_wave: permanent
        _log.warning(
    ...
    if run_id == "system":
        ...
        with contextlib.suppress(Exception):  # pragma: no cover; rule7-exempt: append_fallback_event must not crash  # noqa: E501  expiry_wave: permanent
