...
"""
    ...
    def _row_to_record(self, row: tuple) -> RunRecord:
        return RunRecord(
            run_id=row[0],
            tenant_id=row[1],
            task_contract_json=row[2],
            status=row[3],
            priority=row[4],
            attempt_count=row[5],
            cancellation_flag=bool(row[6]),
    ...
    def upsert(self, record: RunRecord) -> None:
        ...
        with self._lock:
            self._conn.execute(
                ...
                "(run_id, tenant_id, task_contract_json, status, priority, "
    ...
    def get(self, run_id: str) -> RunRecord | None:
        ...
        with self._lock:
            cur = self._conn.execute(
                "SELECT run_id, tenant_id, task_contract_json, status, priority, "
                "attempt_count, cancellation_flag, result_summary, error_summary, "
                "created_at, updated_at "
