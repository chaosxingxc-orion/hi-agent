...
async def handle_create_run(request: Request) -> JSONResponse:
    ...
    if managed_run.outcome == "replayed":
        if managed_run.response_snapshot:
            ...
            except (ValueError, json.JSONDecodeError):  # rule7-exempt: expiry_wave="Wave 30"
                pass
    ...
    return JSONResponse(
        manager.to_dict(run), status_code=201, headers=extra_headers  # type: ignore[arg-type]  expiry_wave: Wave 30 replacement_test: tests/unit/test_routes_runs_rule7.py::test_error_exempt
    )
