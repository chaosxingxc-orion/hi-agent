...
async def handle_create_run(request: Request) -> JSONResponse:
    ...
    rcm = getattr(server, "run_context_manager", None)
    if rcm is not None:
        with contextlib.suppress(Exception):  # rule7-exempt: rcm registration must not block run creation  # noqa: E501  expiry_wave: Wave 17
    ...
    if server.executor_factory is not None:
        ...
        try:
            ...
            with contextlib.suppress(Exception):  # rule7-exempt: cleanup guard must not mask 503  # noqa: E501  expiry_wave: Wave 17
        ...
        def _executor_fn(_managed_run: Any) -> Any:
            try:
                ...
                if rcm is not None:
                    with contextlib.suppress(Exception):  # rule7-exempt: rcm cleanup must not block response  # noqa: E501  expiry_wave: Wave 17
                        rcm.remove(run_id)
    ...
    return JSONResponse(
        manager.to_dict(run), status_code=201, headers=extra_headers  # type: ignore[arg-type]  expiry_wave: Wave 17
    )
    ...
    try:
        with trace_file.open("r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if line:
                    with contextlib.suppress(Exception):  # rule7-exempt: skip corrupt trace lines  # noqa: E501  expiry_wave: Wave 17
