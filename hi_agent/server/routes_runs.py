...
async def handle_create_run(request: Request) -> JSONResponse:
    ...
    return JSONResponse(
        manager.to_dict(run), status_code=201, headers=extra_headers  # type: ignore[arg-type]  expiry_wave: permanent replacement_test: tests/unit/test_routes_runs_rule7.py::test_error_exempt
    )
