...
- Updated validation status to `3858 passed, 13 skipped, 0 failures`.
- Added Workspace Isolation: WorkspaceKey/WorkspacePathHelper, SessionStore/SessionMiddleware, RunManager workspace enforcement, workspace-scoped memory paths (L0–L3 + checkpoints), TeamEventStore, TeamSpace.publish(), GET /team/events, opt-in RunFinalizer auto-sync, acceptance tests 1–20.
- Preserved W13 security hardening: GovernedToolExecutor (central governance gate), PathPolicy (path traversal prevention), URLPolicy (SSRF prevention), CapabilityDescriptor risk metadata, auth posture degraded signal, shell_exec prod-default-disabled, FallbackTaxonomy metrics, ToolCallAuditEvent, JSON-backed RetrievalEngine cache.
- Preserved W13 engineering quality: AsyncBridgeService (shared ThreadPoolExecutor), ContextManager section-level cache, RetrievalEngine index governance, routes_tools_mcp.py extraction, public APIs (LLMBudgetTracker.snapshot, LongTermMemoryGraph.iter_nodes/stats, LLMRouteEngine.set_context_provider), memory store manifest index O(k), SqliteEvidenceStore batch API.
...
tests/                 # 3858 个测试，全部通过（2026-04-19 Workspace Isolation 回归）
...
- **WorkspaceKey / WorkspacePathHelper**（`server/workspace_path.py`）：`private()` / `team()` 统一计算所有存储层路径；`_safe_slug()` 对路径穿越字符进行 SHA-256 哈希防护。
- **SessionStore / SessionMiddleware**（`server/session_store.py`、`server/session_middleware.py`）：SQLite WAL 持久化，`X-Session-Id` 校验，缺失时自动创建并返回。
- **RunManager 工作区执行**：`get_run(run_id, workspace)` 不匹配时返回 `None`（路由层 404）；所有 run 路由均执行工作区所有权校验。
- **内存存储路径注入链**：`ManagedRun → WorkspaceKey → MemoryBuilder → L0/L1/L2/L3/checkpoints` 全链路工作区路径；存储类本身不感知工作区。
- **TeamEventStore / TeamSpace**（`server/team_event_store.py`、`server/team_space.py`）：SQLite WAL + `threading.Lock` 并发安全；`publish()` 携带完整 provenance（source_run/user/session/reason/schema_version）；`GET /team/events` 按 `team_space_id` 隔离。
- **RunFinalizer opt-in Team Sync**：`share_to_team=False`（默认）；私有会话内容不自动泄露到 Team Space。
- **验收测试 1–20**：跨用户 run 访问、SSE 重放、内存路径隔离、Team Space 可见性、路径穿越 fuzz、多进程并发写入均已覆盖。
...
# 列出当前用户的会话
curl http://localhost:8080/sessions -H "X-Tenant-Id: t1" -H "X-User-Id: u1" -H "X-Session-Id: s1"

# 查看 Team Space 事件
curl "http://localhost:8080/team/events?since_id=0" -H "X-Tenant-Id: t1" -H "X-User-Id: u1" -H "X-Session-Id: s1"
