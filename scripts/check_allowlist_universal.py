...
def _parse_allowlists_yaml() -> list[dict]:
    ...
    for line in text.splitlines():
        stripped = line.strip()
        if stripped.startswith("- id:") or stripped.startswith("- symbol:") or stripped.startswith("- path:"):
            if current:
                entries.append(current)
            field = stripped.split(":", 1)
            current = {"_key": field[1].strip() if len(field) > 1 else "", "_source": "allowlists.yaml"}
...
def main() -> int:
    ...
    if args.json:
        ...
        for issue in all_issues:
            print(f"{'FAIL' if issue in expired else 'FAIL'}: {issue}", file=sys.stderr)
        if not all_issues:
