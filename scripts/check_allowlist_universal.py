...
def _parse_allowlists_yaml() -> list[dict]:
    ...
    for line in text.splitlines():
        stripped = line.strip()
        if stripped.startswith("- id:") or stripped.startswith("- symbol:") or stripped.startswith("- path:"):  # noqa: E501  # expiry_wave: Wave 26  # added: W25 baseline sweep
            if current:
                entries.append(current)
            field = stripped.split(":", 1)
            current = {"_key": field[1].strip() if len(field) > 1 else "", "_source": "allowlists.yaml"}  # noqa: E501  # expiry_wave: Wave 26  # added: W25 baseline sweep
...
def _check_in_code_allowlists() -> list[dict]:
    ...
    pattern = re.compile(  # noqa: F841  # expiry_wave: Wave 26  # added: W25 baseline sweep
        r'^\s*["\']([^"\']+)["\'],?\s*#\s*(.*)$',
        re.MULTILINE,
    )
        ...
        for i, line in enumerate(text.splitlines(), 1):
            if re.search(r'["\']\s*(?:,|\]|\))', line) and "ALLOWLIST" in "".join(
                ...
                if not expiry_pattern.search(comment):
                    ...
                    stripped = line.strip()
                    if stripped and not stripped.startswith("#") and '"""' not in stripped:  # noqa: SIM102  # expiry_wave: Wave 26  # added: W25 baseline sweep
...
def main() -> int:
    ...
    if args.json:
        ...
        for issue in all_issues:
            print(f"{'FAIL' if issue in expired else 'FAIL'}: {issue}", file=sys.stderr)  # noqa: RUF034  # expiry_wave: Wave 26  # added: W25 baseline sweep
        if not all_issues:
