...
def main() -> int:
    ...
    if args.json:
        ...
        if all_violations:
            print(f"FAIL: {len(all_violations)} async-resource-in-__init__ violation(s):", file=sys.stderr)  # noqa: E501  # expiry_wave: permanent  # added: W25 baseline sweep
            for v in all_violations:
                print(f"  {v['file']}:{v['line']} [{v['class']}.__init__] {v['type']}(..)", file=sys.stderr)  # noqa: E501  # expiry_wave: permanent  # added: W25 baseline sweep
        else:
