#!/usr/bin/env python3
"""CI gate: every registered CapabilityDescriptor must have an explicit maturity_level (Rule 13 / CL5)."""  # noqa: E501  # expiry_wave: Wave 26  # added: W25 baseline sweep
        ...
        if (
            ...
            for kw in node.keywords:
                if kw.arg == "maturity_level":
                    if isinstance(kw.value, ast.Constant):  # noqa: SIM108  # expiry_wave: Wave 26  # added: W25 baseline sweep
...
def main() -> int:
    ...
    for registry_file in REGISTRY_FILES:
        ...
        for lineno, maturity in instantiations:
            ...
            if maturity is None:
                violations.append(
                    f"{registry_file}:{lineno}: CapabilityDescriptor instantiation missing maturity_level"  # noqa: E501  # expiry_wave: Wave 26  # added: W25 baseline sweep
                )
            elif maturity not in VALID_LEVELS:
                violations.append(
                    f"{registry_file}:{lineno}: maturity_level={maturity!r} not in {sorted(VALID_LEVELS)}"  # noqa: E501  # expiry_wave: Wave 26  # added: W25 baseline sweep
                )
    ...
    print(f"PASS: capability maturity check ({total} CapabilityDescriptor instantiation(s) verified)")  # noqa: E501  # expiry_wave: Wave 26  # added: W25 baseline sweep
    return 0
