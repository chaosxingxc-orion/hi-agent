...
_OWNER_TRAILER = re.compile(r"^Owner:\s*(CO|RO|DX|TE|GOV|AS-CO|AS-RO)\s*$", re.IGNORECASE | re.MULTILINE)  # noqa: E501  # expiry_wave: Wave 26  # added: W25 baseline sweep
...
_CONVENTIONAL = re.compile(r"^(gov|fix|chore|docs|feat|refactor|test|ci|build)(\([^)]*\))?:", re.IGNORECASE)  # noqa: E501  # expiry_wave: Wave 26  # added: W25 baseline sweep
...
def _get_recent_commits(n: int) -> list[dict]:
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True, text=True, timeout=15, cwd=ROOT, encoding="utf-8", errors="replace",  # noqa: E501  # expiry_wave: Wave 26  # added: W25 baseline sweep
        )
...
def _has_owner_tag(commit: dict) -> bool:
    ...
    if _OWNER_TRAILER.search(full):  # noqa: SIM103  # expiry_wave: Wave 26  # added: W25 baseline sweep
        return True
