...
_CLOSURE_PATTERN = re.compile(
    r"\|\s*(?P<row_id>(?:P\d+-\d+|DF-\d+|[A-Z0-9-]+))\s*\|[^|]*\|[^|]*(?:CLOSED|IN PROGRESS|OPEN)[^|]*\|([^|]*)\|",  # noqa: E501  # expiry_wave: permanent  # added: W25 baseline sweep
    re.IGNORECASE,
)
...
_CLOSURE_LEVEL_ENUM = re.compile(
    r"level:\s*(?:" + "|".join(re.escape(l) for l in _VALID_LEVELS) + r")",  # noqa: E741  # expiry_wave: permanent  # added: W25 baseline sweep
    re.IGNORECASE,
)
...
def _is_wave14_plus_notice(path: pathlib.Path, text: str) -> bool:
    ...
    if _CLOSURE_LEVEL_ENUM.search(text):  # noqa: SIM103  # expiry_wave: permanent  # added: W25 baseline sweep
        return True
    return False
