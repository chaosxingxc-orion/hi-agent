...
def main() -> int:
    ...
    if args.json:
        ...
        for issue in issues:
            print(f"FAIL {issue['file']}:{issue['line']}: hardcoded wave string 鈥?use current_wave() from _current_wave.py", file=sys.stderr)  # noqa: E501  # expiry_wave: permanent  # added: W25 baseline sweep
        if not issues:
