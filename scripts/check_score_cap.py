    ...
    for f in NOTICES_DIR.glob("*.md"):
        ...
        for m in score_pattern.finditer(text):
            ...
            if claimed > notice_verified + 0.5:  # allow 0.5 rounding tolerance
                issues.append(f"{f.name}: claims {claimed:.1f} > manifest verified {notice_verified:.1f}")  # noqa: E501  # expiry_wave: Wave 26  # added: W25 baseline sweep
    return issues
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(f"{'PASS' if status == 'pass' else 'FAIL'}: verified={verified:.1f} cap={cap} factors={cap_factors}")  # noqa: E501  # expiry_wave: Wave 26  # added: W25 baseline sweep
        for issue in issues:
