...
def _spawn_server(port: int) -> tuple[subprocess.Popen | None, str]:
    ...
    proc._chaos_log_fp = log_fp  # type: ignore[attr-defined] # expiry_wave: permanent
    return proc, ""
...
def _load_scenario(path: pathlib.Path):  # type: ignore[no-untyped-def] # expiry_wave: permanent
    ...
    try:
        ...
        mod._chaos_load_error = str(exc)  # type: ignore[attr-defined] # expiry_wave: permanent
    return mod
...
def _run_one_scenario(mod, base_url: str, timeout: float) -> dict:  # type: ignore[no-untyped-def] # expiry_wave: permanent
